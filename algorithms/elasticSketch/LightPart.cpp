...
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::insert(uint8_t *key, int size, int delay, int f)
{
    ...
    counters[pos] = (uint8_t)((int)counters[pos] + f); // 统计数据包个数
    sizes[pos] += size; // 统计数据包大小
    delays[pos] += delay; // 统计时延之和
...
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::swap_insert(uint8_t *key, int size, int delay, int f)
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
    uint32_t pos = hash_val % (uint32_t)counter_num;
...
template<int init_mem_in_bytes>
int LightPart<init_mem_in_bytes>::query(uint8_t *key, int *res_cnt, int *res_size) 
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
    uint32_t pos = hash_val % (uint32_t)counter_num;

    *res_cnt = (int)counters[pos];
    *res_size = (int)sizes[pos];
    return 0;
}
